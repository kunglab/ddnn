...
parser.add_argument('-m', '--merge', type=str,  default="max_pool_concat")

args = parser.parse_args()
merge = args.merge

args.ncams = [int(i) for i in args.ncams.split(",")]
mnist = Collection('binary_combining_{}_{}'.format(merge, reduce(lambda x,y: str(x)+str(y), args.ncams)), args.save_dir, input_dims=3, nepochs=args.epochs, verbose=args.verbose)

ncams = args.ncams
mnist.set_model_family(MultiInputEdgeFamily,ninputs=len(ncams),batchsize=700,merge_function=merge)
